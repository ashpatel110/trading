    ...
    for index, from_date in enumerate(expiry_dates[:-1]):
        ...
        while trade_date != next_expiry:
            if trade_date in nifty_50_data:
                ...
                trade_data.append(
                    (trade_date.strftime("%d-%b-%Y"), ohlc.open, strike_price, next_expiry.strftime("%d-%b-%Y"), ce_open, ce_close, pe_open, pe_close, lot_size, profit)
                )
                total_profit += profit
            trade_date += timedelta(days=1)
        with open(f"./output/{from_date.strftime("%d-%b-%Y")}_to_{next_expiry.strftime("%d-%b-%Y")}.csv", "w") as csv_file:
            data_to_store = [
                ("trade_date", "nifty_50_open", "strike_price", "expiry_date", "ce_open", "ce_close", "pe_open", "pe_close", "lot_size", "profit")
            ]
